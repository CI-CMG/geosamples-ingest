...
export default {
  ...
  mutations: {
    ...
    setAge(state, age) {
      state.params.age = age;
    },
    setAgeCode(state, ageCode) {
      state.params.ageCode = ageCode;
    },
    setPage(state, page) {
      state.page = page;
    },
  ...
  actions: {
    ...
    load({ commit }, id) {
      commit('loadRequest');
      return apiService.get(`/age/${encodeURIComponent(encodeSlash(id))}`)
        .then(
          (response) => {
    ...
    save({ commit }, { provider, id }) {
      commit('saveRequest');
      const req = id ? () => apiService.put(`/age/${encodeURIComponent(encodeSlash(id))}`, provider) : () => apiService.post('/age', provider);
      return req()
        .then(
          (response) => {
              ...
              if (data) {
                ...
                if (formErrors) {
                  ...
                  paths.forEach((path) => {
                    ...
                    commit('ageForm/setTouched', { path, touched: false }, { root: true });
                    commit('ageForm/setError', { path, error: message }, { root: true });
                  });
                }
              }
            }
    ...
    delete({ commit }, id) {
      ...
      return apiService.delete(`/age/${encodeURIComponent(encodeSlash(id))}`)
        .then(
          (response) => {
