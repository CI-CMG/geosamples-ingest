...
export default {
  ...
  mutations: {
    ...
    setCruiseYear(state, cruiseYear) {
      state.params.cruiseYear = cruiseYear;
    },
    setPlatform(state, platform) {
      state.params.platform = platform;
    },
    setLeg(state, leg) {
      state.params.leg = leg;
    },
    setLegName(state, legName) {
      state.params.legName = legName;
    },
  ...
  actions: {
    ...
    searchPage({ commit, state }) {
      ...
      return apiService.get('/cruise-link', {
        params:
          {
            ...params,
            page: state.page,
    ...
    load({ commit }, id) {
      commit('loadRequest');
      return apiService.get(`/cruise-link/${id}`)
        .then(
          (response) => {
    ...
    save({ commit }, { provider, id }) {
      commit('saveRequest');
      const req = id ? () => apiService.put(`/cruise-link/${id}`, provider) : () => apiService.post('/cruise-link', provider);
      return req()
        .then(
          (response) => {
              ...
              if (data) {
                ...
                if (formErrors) {
                  ...
                  paths.forEach((path) => {
                    ...
                    commit('cruiseLinkForm/setTouched', { path, touched: false }, { root: true });
                    commit('cruiseLinkForm/setError', { path, error: message }, { root: true });
                  });
                }
              }
            }
    ...
    delete({ commit }, id) {
      ...
      return apiService.delete(`/cruise-link/${id}`)
        .then(
          (response) => {
