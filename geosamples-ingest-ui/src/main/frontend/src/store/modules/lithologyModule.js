...
export default {
  ...
  mutations: {
    ...
    setLithology(state, lithology) {
      state.params.lithology = lithology;
    },
    setLithologyCode(state, lithologyCode) {
      state.params.lithologyCode = lithologyCode;
    },
    setPage(state, page) {
      state.page = page;
    },
  ...
  actions: {
    ...
    searchPage({ commit, state }) {
      ...
      return apiService.get('/lithology', {
        params:
          {
            ...params,
            page: state.page,
    ...
    load({ commit }, id) {
      commit('loadRequest');
      return apiService.get(`/lithology/${encodeURIComponent(encodeSlash(id))}`)
        .then(
          (response) => {
    ...
    save({ commit }, { provider, id }) {
      commit('saveRequest');
      const req = id ? () => apiService.put(`/lithology/${encodeURIComponent(encodeSlash(id))}`, provider) : () => apiService.post('/lithology', provider);
      return req()
        .then(
          (response) => {
              ...
              if (data) {
                ...
                if (formErrors) {
                  ...
                  paths.forEach((path) => {
                    ...
                    commit('lithologyForm/setTouched', { path, touched: false }, { root: true });
                    commit('lithologyForm/setError', { path, error: message }, { root: true });
                  });
                }
              }
            }
    ...
    delete({ commit }, id) {
      ...
      return apiService.delete(`/lithology/${encodeURIComponent(encodeSlash(id))}`)
        .then(
          (response) => {
