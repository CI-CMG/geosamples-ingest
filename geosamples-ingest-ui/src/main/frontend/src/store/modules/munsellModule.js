...
export default {
  ...
  mutations: {
    ...
    setMunsell(state, munsell) {
      state.params.munsell = munsell;
    },
    setMunsellCode(state, munsellCode) {
      state.params.munsellCode = munsellCode;
    },
    setPage(state, page) {
      state.page = page;
    },
  ...
  actions: {
    ...
    load({ commit }, id) {
      commit('loadRequest');
      return apiService.get(`/munsell/${encodeURIComponent(encodeSlash(id))}`)
        .then(
          (response) => {
    ...
    save({ commit }, { provider, id }) {
      commit('saveRequest');
      const req = id ? () => apiService.put(`/munsell/${encodeURIComponent(encodeSlash(id))}`, provider) : () => apiService.post('/munsell', provider);
      return req()
        .then(
          (response) => {
              ...
              if (data) {
                ...
                if (formErrors) {
                  ...
                  paths.forEach((path) => {
                    ...
                    commit('munsellForm/setTouched', { path, touched: false }, { root: true });
                    commit('munsellForm/setError', { path, error: message }, { root: true });
                  });
                }
              }
            }
    ...
    delete({ commit }, id) {
      ...
      return apiService.delete(`/munsell/${encodeURIComponent(encodeSlash(id))}`)
        .then(
          (response) => {
