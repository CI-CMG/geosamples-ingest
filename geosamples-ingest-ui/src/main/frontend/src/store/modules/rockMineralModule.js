...
export default {
  ...
  mutations: {
    ...
    setRockMineral(state, rockMineral) {
      state.params.rockMineral = rockMineral;
    },
    setRockMineralCode(state, rockMineralCode) {
      state.params.rockMineralCode = rockMineralCode;
    },
    setPage(state, page) {
      state.page = page;
    },
  ...
  actions: {
    ...
    searchPage({ commit, state }) {
      ...
      return apiService.get('/rock-mineral', {
        params:
          {
            ...params,
            page: state.page,
    ...
    load({ commit }, id) {
      commit('loadRequest');
      return apiService.get(`/rock-mineral/${encodeURIComponent(id)}`)
        .then(
          (response) => {
    ...
    save({ commit }, { provider, id }) {
      commit('saveRequest');
      const req = id ? () => apiService.put(`/rock-mineral/${encodeURIComponent(id)}`, provider) : () => apiService.post('/rock-mineral', provider);
      return req()
        .then(
          (response) => {
    ...
    delete({ commit }, id) {
      ...
      return apiService.delete(`/rock-mineral/${encodeURIComponent(id)}`)
        .then(
          (response) => {
