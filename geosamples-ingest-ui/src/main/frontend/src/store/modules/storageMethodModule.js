...
export default {
  ...
  mutations: {
    ...
    setStorageMethod(state, storageMethod) {
      state.params.storageMethod = storageMethod;
    },
    setStorageMethodCode(state, storageMethodCode) {
      state.params.storageMethodCode = storageMethodCode;
    },
    setPage(state, page) {
      state.page = page;
    },
  ...
  actions: {
    ...
    searchPage({ commit, state }) {
      ...
      return apiService.get('/storage-method', {
        params:
          {
            ...params,
            page: state.page,
    ...
    load({ commit }, id) {
      commit('loadRequest');
      return apiService.get(`/storage-method/${encodeURIComponent(encodeSlash(id))}`)
        .then(
          (response) => {
    ...
    save({ commit }, { provider, id }) {
      commit('saveRequest');
      const req = id ? () => apiService.put(`/storage-method/${encodeURIComponent(encodeSlash(id))}`, provider) : () => apiService.post('/storage-method', provider);
      return req()
        .then(
          (response) => {
              ...
              if (data) {
                ...
                if (formErrors) {
                  ...
                  paths.forEach((path) => {
                    ...
                    commit('storageMethodForm/setTouched', { path, touched: false }, { root: true });
                    commit('storageMethodForm/setError', { path, error: message }, { root: true });
                  });
                }
              }
            }
    ...
    delete({ commit }, id) {
      ...
      return apiService.delete(`/storage-method/${encodeURIComponent(encodeSlash(id))}`)
        .then(
          (response) => {
