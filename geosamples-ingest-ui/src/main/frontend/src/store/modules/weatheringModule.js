...
export default {
  ...
  mutations: {
    ...
    setWeathering(state, weathering) {
      state.params.weathering = weathering;
    },
    setWeatheringCode(state, weatheringCode) {
      state.params.weatheringCode = weatheringCode;
    },
    setPage(state, page) {
      state.page = page;
    },
  ...
  actions: {
    ...
    searchPage({ commit, state }) {
      ...
      return apiService.get('/weathering', {
        params:
          {
            ...params,
            page: state.page,
    ...
    load({ commit }, id) {
      commit('loadRequest');
      return apiService.get(`/weathering/${encodeURIComponent(encodeSlash(id))}`)
        .then(
          (response) => {
    ...
    save({ commit }, { provider, id }) {
      commit('saveRequest');
      const req = id ? () => apiService.put(`/weathering/${encodeURIComponent(encodeSlash(id))}`, provider) : () => apiService.post('/weathering', provider);
      return req()
        .then(
          (response) => {
              ...
              if (data) {
                ...
                if (formErrors) {
                  ...
                  paths.forEach((path) => {
                    ...
                    commit('weatheringForm/setTouched', { path, touched: false }, { root: true });
                    commit('weatheringForm/setError', { path, error: message }, { root: true });
                  });
                }
              }
            }
    ...
    delete({ commit }, id) {
      ...
      return apiService.delete(`/weathering/${encodeURIComponent(encodeSlash(id))}`)
        .then(
          (response) => {
